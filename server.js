...
app.post("/render", upload.any(), async (req, res) => {
  ...
  try {
    ...
    // Process all images/icons in global order matching the flat imageFiles array
    let globalIndex = 0;
    ...
    for (const section of sectionsMeta) {
      ...
      for (const meta of section.items) {
        ...
        if (!file) { globalIndex++; continue; }

        console.log(`[server] Processing image ${globalIndex + 1}/${imageFiles.length}: ${file.originalname}`);
        const processedImage = await processImage(file.path);

        const iconFile = allFiles.find(f => f.fieldname === `icon_${globalIndex}`);
        let iconImage;
        if (iconFile) {
        ...
        sectionItems.push({
          ...
          rank: parseInt(meta.rank, 10) || (imageFiles.length - globalIndex),
    ...
    if (processedSections.length === 1) {
      ...
      for (let i = 0; i < processedSections.length; i++) {
        const segPath = path.join(__dirname, "output", `seg_${timestamp}_${i}.mp4`);
        segPaths.push(segPath);
        console.log(`[server] Rendering section ${i + 1}/${processedSections.length}`);
        await render(processedSections[i], segPath);
      }

      console.log(`[server] Concatenating ${segPaths.length} segments with fade transitions`);
      await concatenateWithFade(segPaths, processedSections.map(s => segDuration(s.length)), outputPath);
    }

    res.json({ success: true, video: filename, url: `/output/${filename}` });
...
function concatenateWithFade(segPaths, durations, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 1; i < segPaths.length; i++) {
      const offset = accumulated + durations[i - 1] - FADE * i;
      const nextTag = i === segPaths.length - 1 ? "[vout]" : `[v${i}]`;
      filterStr += `${prevTag}[${i}:v]xfade=transition=fade:duration=${FADE}:offset=${offset.toFixed(3)}`;
      filterStr += nextTag;
      if (i < segPaths.length - 1) filterStr += ";";
    ...
    cmd
      ...
      .save(outputPath)
      .on("start", c => console.log("[ffmpeg concat] cmd:", c.substring(0, 200) + "..."))
      .on("end", () => { console.log("[ffmpeg concat] Done"); resolve(); })
      .on("error", (err, _out, stderr) => {
        console.error("[ffmpeg concat] ERROR:", err.message, stderr);
        reject(err);
      });
  });
}
